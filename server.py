...
def deviceFiltering(devicesJson):
...
def get_devices():
    ...
    result  = requests.get(settings.api_base_url+"me/player/devices",headers=headers)
    devices = result.json()
    devices = deviceFiltering(devices)
    if request.method == "POST":
       for device in devices:
          if request.form['deviceButton'] == device['name']:
             settings.device_id=device['id']
    return render_template("devices.html",dev=devices)
...
def choosePlaylist(number):
    headers = get_auth_header(settings.currToken)
    result = requests.get(settings.api_base_url+"me/playlists",headers=headers)
    playlists = result.json()
    namesId = playlistsFiltering(playlists)
    for i,playlist in enumerate(namesId,1):
      if i==number:
         settings.currentPlaylist = playlist["id"]

def setVolume(number):
   headers = get_auth_header(settings.currToken)
   requests.put(settings.api_base_url+"me/player/volume?volume_percent="+str(number)+"&device_id="+settings.device_id,headers=headers)
   print("Changed volume to: ",number)
...
def run():
    load_dotenv()
    app.run(host='192.168.184.82',port=5000)
